 ...
 * </ol>
 */
class Statm : ProcFile {
    companion object CREATOR : Parcelable.Creator<Statm> {
        @Throws(IOException::class)
        fun get(pId: Int): Statm {
            return Statm(String.format(Locale.ENGLISH, "/proc/%d/statm", pId))
        }

        override fun createFromParcel(parcel: Parcel): Statm {
            return Statm(parcel)
        }

        override fun newArray(size: Int): Array<Statm?> {
     ...
     */
    fun getSize(): Long {
        return fields[0].toLong() * 1024
    }

    /**
     * @return the resident set size in bytes
     */
    fun getResidentSetSize(): Long {
        return fields[1].toLong() * 1024
    }
