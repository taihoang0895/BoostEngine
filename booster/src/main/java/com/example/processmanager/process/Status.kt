 ...
 * </ol>
 */
class Status : ProcFile {

    companion object CREATOR : Parcelable.Creator<Status> {
        @Throws(IOException::class)
        fun get(pId: Int): Status {
            return Status(String.format(Locale.ENGLISH, "/proc/%d/status", pId))
        }
    ...
    fun getValue(fieldName: String): String? {
        ...
        for (line in lines) {
            if (line.startsWith("$fieldName:")) {
                return line.split("$fieldName:".toRegex()).toTypedArray()[1].trim { it <= ' ' }
            }
        }
        return null
    }
    ...
    fun getUid(): Int {
        return try {
            getValue("Uid")!!.split("\\s+".toRegex()).toTypedArray()[0].toInt()
        } catch (e: Exception) {
            -1
        }
    }
    ...
    fun getGid(): Int {
        return try {
            getValue("Gid")!!.split("\\s+".toRegex()).toTypedArray()[0].toInt()
